...
Emit simulated offboarding events and destroy demo-fleet stack.

Options:
  --stack <name>               Pulumi stack (default: demo-fleet)
  --iac-dir <path>             Pulumi project dir (default: infra/demo-fleet)
  --inventory-file <path>      Inventory json file (default: .data/demo-fleet-target-inventory.json)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "demo-fleet-down: unknown argument: $1" >&2
      usage >&2
      exit 2
      ;;
  esac
...
if ! command -v pulumi >/dev/null 2>&1; then
  echo "demo-fleet-down: Pulumi CLI is required." >&2
  exit 1
fi
if [[ ! -d "${IAC_DIR}" ]]; then
  echo "demo-fleet-down: missing IaC directory: ${IAC_DIR}" >&2
  exit 1
fi
...
if [[ "${SKIP_EVENTS}" != "true" ]]; then
  ...
  if pulumi stack output --stack "${STACK}" mappoTargetInventory --json >"${INVENTORY_FILE}" 2>/dev/null; then
    echo "demo-fleet-down: refreshed inventory ${INVENTORY_FILE}"
  fi
fi
...
if [[ "${SKIP_EVENTS}" != "true" && -n "${API_BASE_URL}" && -f "${INVENTORY_FILE}" ]]; then
  ...
  "${ROOT_DIR}/scripts/marketplace_ingest_events.sh" \
    ...
    --event-id-prefix "evt-demo-fleet-down" \
    --source-label "demo-fleet-down" \
    "${INGEST_ARGS[@]}"
  echo "demo-fleet-down: marketplace offboarding ingest complete."
elif [[ "${SKIP_EVENTS}" != "true" ]]; then
  echo "demo-fleet-down: skipped offboarding ingest (missing api-base-url or inventory file)."
fi
...
echo "demo-fleet-down: stack destroyed."
