...
Delete MAPPO targets listed in the App Service fleet Pulumi inventory.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "appservice-fleet-delete-targets: unknown argument: $1" >&2
      usage >&2
      exit 2
      ;;
  esac
...
if [[ -z "${API_BASE_URL}" ]]; then
  echo "appservice-fleet-delete-targets: --api-base-url (or MAPPO_API_BASE_URL) is required." >&2
  exit 2
fi
