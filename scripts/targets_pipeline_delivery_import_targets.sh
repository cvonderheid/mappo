...
Import Pipeline Delivery Demo Targets targets into MAPPO from Pulumi inventory.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    *)
      echo "targets-pipeline-delivery-import-targets: unknown argument: $1" >&2
      usage >&2
      exit 2
      ;;
  esac
...
if [[ -z "${API_BASE_URL}" ]]; then
  echo "targets-pipeline-delivery-import-targets: --api-base-url (or MAPPO_API_BASE_URL) is required." >&2
  exit 2
fi
...
ARGS=(
  ...
  --source-label "targets-pipeline-delivery-iac-import"
)
