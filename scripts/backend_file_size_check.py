...
def main() -> int:
    ...
    root = Path(args.root)
    if not root.exists():
        print(f"backend-file-size-check: FAIL root not found: {root}")
        return 1
    ...
    if violations:
        print(
            f"backend-file-size-check: FAIL {len(violations)} files exceed {args.max_lines} lines "
            f"(checked {checked})"
        )
    ...
    print(f"backend-file-size-check: PASS checked={checked} max_lines={args.max_lines}")
    return 0
