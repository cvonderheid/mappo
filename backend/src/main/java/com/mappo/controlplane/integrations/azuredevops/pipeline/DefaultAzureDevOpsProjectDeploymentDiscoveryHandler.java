...
@Service
public class DefaultAzureDevOpsProjectDeploymentDiscoveryHandler implements AzureDevOpsProjectDeploymentDiscoveryHandler {
    ...
    public DefaultAzureDevOpsProjectDeploymentDiscoveryHandler(
    ...
    public ProjectAdoPipelineDiscoveryResultRecord discoverAdoPipelines(
        ...
        if (organization.isBlank() || adoProject.isBlank()) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Select an Azure DevOps project before MAPPO can discover pipelines.");
        }
        try {
            return new ProjectAdoPipelineDiscoveryResultRecord(
                ...
                organization,
                adoProject,
                azureDevOpsDiscoveryGateway.discoverPipelines(organization, adoProject, personalAccessToken, nameContains)
            );
    ...
    public ProjectAdoRepositoryDiscoveryResultRecord discoverAdoRepositories(
        ...
        String nameContains = normalize(request == null ? null : request.nameContains());
        if (organization.isBlank() || adoProject.isBlank()) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Select an Azure DevOps project before MAPPO can discover repositories.");
        }
        try {
            return new ProjectAdoRepositoryDiscoveryResultRecord(
                ...
                organization,
                adoProject,
                azureDevOpsDiscoveryGateway.discoverRepositories(organization, adoProject, personalAccessToken, nameContains)
            );
        } catch (IllegalArgumentException exception) {
            throw new ApiException(HttpStatus.BAD_REQUEST, exception.getMessage());
        } catch (AzureDevOpsDiscoveryException exception) {
    ...
    public ProjectAdoBranchDiscoveryResultRecord discoverAdoBranches(
        ...
        String repository = firstNonBlank(request == null ? null : request.repository(), config.repository());
        String personalAccessToken = resolveAdoPersonalAccessToken(definition, request == null ? null : request.providerConnectionId());
        String nameContains = normalize(request == null ? null : request.nameContains());
        if (organization.isBlank() || adoProject.isBlank()) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Select an Azure DevOps project before MAPPO can discover branches.");
        }
        if (repositoryId.isBlank() && repository.isBlank()) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Select an Azure DevOps repository before MAPPO can discover branches.");
        }
        try {
    ...
    private PipelineTriggerDriverConfig requireAdoPipelineTriggerConfig(ProjectDefinition definition) {
        if (definition.deploymentDriver() != ProjectDeploymentDriverType.pipeline_trigger) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Project deployment driver must be pipeline_trigger for Azure DevOps pipeline discovery.");
        }
        if (!(definition.deploymentDriverConfig() instanceof PipelineTriggerDriverConfig config)) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Project deployment driver configuration is not pipeline_trigger-compatible.");
        }
        String pipelineSystem = normalize(config.pipelineSystem());
        if (!"azure_devops".equalsIgnoreCase(pipelineSystem)) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "This project must use the Azure DevOps pipeline system before MAPPO can discover Azure DevOps resources.");
        }
        return config;
    }
    ...
    private String resolveAdoPersonalAccessToken(ProjectDefinition definition, String requestProviderConnectionId) {
        ...
        if (providerConnectionId.isBlank()) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Select a deployment connection before MAPPO can discover Azure DevOps resources.");
        }
        ...
        if (connection.provider() == null || !"azure_devops".equalsIgnoreCase(connection.provider().name())) {
            throw new ApiException(HttpStatus.BAD_REQUEST, "Deployment connection " + providerConnectionId + " is not configured for Azure DevOps.");
        }
