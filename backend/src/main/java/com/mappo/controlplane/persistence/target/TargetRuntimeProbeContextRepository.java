...
public class TargetRuntimeProbeContextRepository {
    ...
    public List<TargetRuntimeProbeContextRecord> listRuntimeProbeContexts() {
        return listRuntimeProbeContexts(null);
    }

    public List<TargetRuntimeProbeContextRecord> listRuntimeProbeContextsForProject(String projectId) {
        return listRuntimeProbeContexts(projectId);
    }

    private List<TargetRuntimeProbeContextRecord> listRuntimeProbeContexts(String projectId) {
        ...
        var rows = dsl.select(
            ...
            .where(normalizedProjectId.isBlank() ? org.jooq.impl.DSL.trueCondition() : TARGETS.PROJECT_ID.eq(normalizedProjectId))
    ...
    private RuntimeProbeConfig parseRuntimeProbeConfig(ProjectRuntimeHealthProviderType providerType, String value) {
        var config = runtimeHealthProviderConfigRegistry.parse(providerType, value);
        return new RuntimeProbeConfig(config.path(), config.expectedStatus(), config.timeoutMs());
    }
