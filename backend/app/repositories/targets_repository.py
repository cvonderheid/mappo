...
from app.db.generated.models import TargetRegistrations, Targets, TargetTags
from app.modules.schemas import Target, TargetRegistrationRecord
...
class TargetsRepository:
    ...
    def load_targets(self) -> dict[str, Target]:
        with self._session_factory() as session:
            ...
            tags_by_target = self._load_target_tags_map(session, target_ids=target_ids)
    ...
    def load_target_registrations(self) -> dict[str, TargetRegistrationRecord]:
        with self._session_factory() as session:
    ...
    def replace_targets(self, *, targets: list[Target], updated_at: datetime) -> None:
        with self._session_factory() as session:
            ...
            for target in sorted(targets, key=lambda item: item.id):
                ...
                for tag_key, tag_value in sorted(target.tags.items()):
                    session.add(
                        TargetTags(target_id=target.id, tag_key=tag_key, tag_value=tag_value)
                    )
            session.commit()

    def save_target(self, *, target: Target, updated_at: datetime) -> None:
        with self._session_factory() as session:
    ...
    def delete_target_registration(self, *, target_id: str) -> None:
        with self._session_factory() as session:
            session.execute(
