...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Import MAPPO targets from Pulumi inventory JSON output."
    )
    parser.add_argument(
        "--file",
        type=Path,
        default=DEFAULT_INVENTORY_PATH,
        help=f"Path to Pulumi target inventory JSON (default: {DEFAULT_INVENTORY_PATH})",
    )
    parser.add_argument(
        "--clear-runs",
        action="store_true",
    ...
    try:
        ...
        imported = [
            _to_target(row, existing_by_id=by_existing_id)
